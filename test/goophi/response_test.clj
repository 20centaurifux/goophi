...
(deftest is-response?
  ...
  (testing "input-stream"
    (is (Response? (str->stream
                    "hello world"))))
  (testing "menu entity"
    (is (Response? (menu-entity
                    "hello world"))))
  (testing "text-file entity"
    (is (Response? (text-file-entity
                    (str->stream "hello world")))))
  (testing "binary entity"
    (is (Response? (binary-entity
                    (byte-array [1 3 3 7]))))))
...
(deftest dump
  ...
  (testing "hex"
    (let [dump (-> (str->stream "hello world")
                   ...
                   with-out-str
                   s/trim)]
      (is (= "0x68 0x65 0x6c 0x6c 0x6f 0x20 0x77 0x6f 0x72 0x6c 0x64"
             dump)))))
    ...
    (testing "replace tab"
      (let [lines (-> (str->stream "hello\tworld")
        ...
        (is (not (.contains (first lines)
                            "\t")))
        (is (some? (re-matches #"hello\s+world"
                               (first lines))))
        (is (= "."
               (second lines)))))))
